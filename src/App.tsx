...
const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  `transition-colors duration-200 font-medium text-sm ${
    isActive
      ? 'text-primary dark:text-primary-fixed-dim font-bold border-b-2 border-primary dark:border-primary-fixed-dim pb-0.5'
      : 'text-on-surface-variant dark:text-gray-300 hover:text-primary dark:hover:text-primary-fixed-dim'
  }`
...
function Home({
  ...
  uptime: number
  pings: { id: number; server: string; time: string; ping: number; status: string }[]
  uptimeRef: React.RefObject<null>
  isUptimeInView: boolean
  activeTab: string
  ...
  return (
    <main className="flex-grow">
      ...
      <section className="relative pt-12 md:pt-20 pb-20 md:pb-28 overflow-hidden bg-gradient-to-b from-[#ffffff] to-[#f8fafc] dark:from-[#0b0f19] dark:to-[#0f172a] transition-colors duration-500">
        <div className="absolute inset-0 circuit-bg opacity-30 dark:opacity-20 pointer-events-none" />
        ...
        <div className="max-w-container-max mx-auto px-gutter relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            >
              ...
              <span className="font-label-sm text-xs uppercase tracking-widest font-bold">Empowering SMEs</span>
            </motion.div>
            ...
            <p className="font-body-lg text-lg text-on-surface-variant dark:text-gray-300 max-w-xl">
              We provide robust, scalable, and secure technological infrastructures designed to accelerate your
              business growth, automate operations, and defend your assets.
            </p>
          ...
          >
            <div className="absolute w-[120%] h-[120%] bg-primary-container/10 dark:bg-primary-fixed-dim/5 rounded-full blur-[100px] pointer-events-none z-0" />
        ...
        <div className="max-w-container-max mx-auto px-gutter">
          ...
          <div className="text-center mb-16 space-y-4">
            ...
            >
              ...
              <div className="w-16 h-1 bg-primary dark:bg-primary-fixed-dim mx-auto mt-4 rounded-full" />
            </motion.div>
            <p className="font-body-lg text-lg text-on-surface-variant dark:text-gray-300 max-w-2xl mx-auto">
              Comprehensive IT services tailored to meet the exacting demands of modern enterprise environments.
            </p>
        ...
        <div className="max-w-container-max mx-auto px-gutter grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <div className="space-y-4">
              ...
              <div className="w-16 h-1 bg-primary dark:bg-primary-fixed-dim rounded-full" />
            </div>

            <p className="font-body-lg text-lg text-on-surface-variant dark:text-gray-300 leading-relaxed">
              We do not just patch IT problems; we architect robust, proactive digital environments that scale
              alongside your operations. Our expert consultants ensure zero bottlenecking.
            </p>

            <ul className="space-y-6 pt-4">
              {[
                { title: 'Rapid Adherence SLA', desc: 'Instant ticket logging with 15-minute response times for critical outages.', icon: 'flash_on' },
                { title: 'Highly Certified Engineers', desc: 'Our staff hold Cisco, AWS, and cyber defense certifications.', icon: 'verified' },
                { title: 'Proactive Infrastructure Defense', desc: '24/7 endpoint shielding to block threats before they hit your network.', icon: 'security' },
              ].map(item => (
                <motion.li
                  whileHover={{ x: 6 }}
                  ...
                  <AnimatePresence>
                    {pings.map(ping => (
                      ...
                      >
                        ...
                        <span className="text-slate-500 font-mono flex-grow border-b border-dotted border-slate-800 mx-1" />
                        <span className="font-mono text-right shrink-0">
                          {ping.ping}ms -{' '}
                          <span className={ping.status === 'OK' ? 'text-green-400' : 'text-amber-400 font-bold'}>
                            {ping.status}
                          </span>
                        </span>
                      </motion.div>
...
export default function App() {
  ...
  // Uptime counter
  useEffect(() => {
    if (!isUptimeInView) return
    let start = 0
    const end = 99.99
    const timer = setInterval(() => {
      start += end / (2000 / 30)
      if (start >= end) { setUptime(end); clearInterval(timer) }
      else setUptime(Number(start.toFixed(2)))
    }, 30)
  ...
  useEffect(() => {
    ...
    const make = () => ({
      ...
      status: Math.random() > 0.005 ? 'OK' : 'DEGRADED',
    })
    setPings([make(), make(), make()])
    const iv = setInterval(() => setPings(p => [make(), ...p.slice(0, 4)]), 4000)
      ...
      <nav className="bg-surface/90 dark:bg-[#0b0f19]/90 backdrop-blur-md w-full top-0 sticky border-b border-outline-variant/30 dark:border-outline/20 z-50 transition-colors duration-300">
        <div className="flex justify-between items-center max-w-container-max mx-auto px-gutter py-4">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <button onClick={toggleDarkMode} className="p-2 rounded-full hover:bg-surface-container dark:hover:bg-slate-800 text-on-surface-variant dark:text-gray-300 transition-colors duration-300" aria-label="Toggle dark mode">
              <span className="material-symbols-outlined flex items-center justify-center">{darkMode ? 'light_mode' : 'dark_mode'}</span>
            </button>
            <button onClick={() => setIsQuoteModalOpen(true)} className="bg-primary-container text-on-primary-container px-6 py-2.5 rounded-full font-bold hover:bg-primary hover:text-on-primary transition-all duration-300 shadow-sm hover:shadow-md cursor-pointer">
              Get a Quote
            </button>
          </div>
          ...
          <div className="flex items-center space-x-2 md:hidden">
            ...
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-primary dark:text-primary-fixed-dim focus:outline-none hover:bg-surface-container dark:hover:bg-slate-800 rounded-full" aria-label="Toggle mobile menu">
              <span className="material-symbols-outlined flex items-center justify-center">{isMobileMenuOpen ? 'close' : 'menu'}</span>
            </button>
          </div>
        </div>
              ...
              <div className="flex flex-col px-gutter py-6 space-y-4">
                {navLinks.map(l => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `text-lg font-semibold flex items-center gap-2 transition-colors ${isActive ? 'text-primary dark:text-primary-fixed-dim' : 'text-on-surface-variant dark:text-gray-300 hover:text-primary dark:hover:text-primary-fixed-dim'}`
                    }
                  >
                    {l.label}
                  </NavLink>
        ...
        <div className="max-w-container-max mx-auto px-gutter py-16 grid grid-cols-1 md:grid-cols-12 gap-10">
          ...
          <div className="md:col-span-4 space-y-4 text-left">
            ...
            <p className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 leading-relaxed max-w-sm">
              © 2026 NETFIX IT SOLUTION. Empowering small and medium enterprises through premium technical partnerships.
            </p>
          </div>
          ...
          <div className="md:col-span-2 space-y-4 text-left">
            <h4 className="font-body-lg text-base font-bold text-on-surface dark:text-white uppercase tracking-wider">Company</h4>
            <ul className="space-y-2.5">
              <li>
                <Link to="/about" className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 hover:text-primary dark:hover:text-primary-fixed-dim transition-colors duration-200">
                  About Us
                </Link>
              </li>
              <li>
                <a href="#" className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 hover:text-primary dark:hover:text-primary-fixed-dim transition-colors duration-200">Privacy Policy</a>
              </li>
              <li>
                <a href="#" className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 hover:text-primary dark:hover:text-primary-fixed-dim transition-colors duration-200">Terms of Service</a>
              </li>
            </ul>
          </div>
          ...
          <div className="md:col-span-3 space-y-4 text-left">
            ...
            <div className="space-y-3">
              <a href="mailto:info@netfix.com" className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 flex items-center hover:text-primary dark:hover:text-primary-fixed-dim transition-colors">
                ...
                {!formSubmitted ? (
                  <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-4">
                      {[
                        ...
                        <div key={f.name}>
                          ...
                          <input
                            ...
                            className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-on-surface dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all"
                          />
                        </div>
                      ))}
                      ...
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Project Brief</label>
                        <textarea name="message" rows={3} value={formData.message} onChange={handleFormChange} placeholder="Outline your requirements..." className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-on-surface dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all" />
                      </div>

                      <button type="submit" className="w-full bg-primary text-on-primary dark:bg-primary-fixed-dim dark:text-on-primary-fixed py-3.5 rounded-xl font-bold hover:shadow-lg transition-all duration-300 shadow-md cursor-pointer flex items-center justify-center gap-2 mt-2">
                  ...
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                    <div className="w-16 h-16 bg-green-500/10 text-green-500 rounded-full flex items-center justify-center">
                      <span className="material-symbols-outlined text-4xl animate-bounce">check_circle</span>
                    </div>
                    <h3 className="font-display-lg text-2xl font-extrabold text-on-surface dark:text-white">Quote Request Received!</h3>
                    <p className="font-body-md text-sm text-on-surface-variant dark:text-gray-400 max-w-sm">
                      Thank you, <span className="font-bold text-on-surface dark:text-white">{formData.name}</span>. We'll reach out to <span className="font-bold text-on-surface dark:text-white">{formData.email}</span> shortly.
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        {selectedService && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white dark:bg-[#121826] border border-slate-200 dark:border-slate-800/80 rounded-2xl max-w-lg w-full p-8 relative shadow-2xl text-left"
            >
              <button onClick={() => setSelectedService(null)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-white rounded-full hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors">
                <span className="material-symbols-outlined">close</span>
              </button>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-primary/10 text-primary dark:bg-primary-fixed-dim/10 dark:text-primary-fixed-dim rounded-xl flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>{selectedService.icon}</span>
                  </div>
                  <div>
                    <h3 className="font-display-lg text-2xl font-extrabold text-on-surface dark:text-white">{selectedService.title}</h3>
                    <span className="inline-block px-3 py-1 mt-1 bg-primary/10 text-primary dark:bg-primary-fixed-dim/10 dark:text-primary-fixed-dim font-label-sm text-xs font-semibold rounded-full">{selectedService.tag}</span>
                  </div>
                </div>

                <p className="font-body-md text-sm text-on-surface-variant dark:text-gray-300 leading-relaxed">{selectedService.desc}</p>

                <div className="space-y-3">
                  <h4 className="font-body-lg text-sm font-bold text-on-surface dark:text-white uppercase tracking-wider">Core Technical Features</h4>
                  <ul className="space-y-2.5">
                ...
                <div className="pt-4 flex gap-4">
                  <button onClick={() => { setSelectedService(null); setIsQuoteModalOpen(true) }} className="flex-grow bg-primary text-on-primary dark:bg-primary-fixed-dim dark:text-on-primary-fixed py-3 rounded-xl font-bold hover:opacity-90 transition-all duration-300 shadow-md cursor-pointer">
                    Request Installation
                  </button>
                  <button onClick={() => setSelectedService(null)} className="px-6 py-3 border border-slate-200 dark:border-slate-800 text-on-surface dark:text-white rounded-xl font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors">
                    Close
                  </button>
                </div>
              </div>
